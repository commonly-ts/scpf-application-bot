...
export async function HandleAcceptApplicationButton(
    client: Client<true>,
    ...
    try {
        ...
        await interaction.editReply({
            embeds: [
                new EmbedBuilder()
                    .setColor("Green")
                    .setDescription(`Successfully accepted ${application.robloxUsername}'s application`)
            ]
        });
        ...
        const userid = Number(application.robloxId);

        const resultsChannel = guild.channels.cache.get("1157173368248864860") as TextChannel | undefined;
        const playerThumbnail = await noblox.getPlayerThumbnail(userid, 420, "png", true, "bust");

        const playerRank = await noblox.getRankInGroup(32906850, userid);

        if (application.acceptanceRank && playerRank > 0 && playerRank < application.acceptanceRank) {
            await noblox.setRank(32906850, userid, application.acceptanceRank);
        }
            ...
            await fetch(`https://api.blox.link/v4/public/guilds/${guild.id}/roblox-to-discord/${application.robloxId}`, {
                ...
                .then(async (data) => {
                    ...
                    const resultEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`${application.robloxUsername} has been accepted for the ${application.applicationName} application.`)
